...
public class InventoryService {
    ...
    public void addInventory(Inventory inventory) {
        boolean exists = inventoryRepository.existsById(inventory.getProductId());
        if (exists){
            throw new IllegalStateException("Product id " + inventory.getProductId() + " already exists.");
        }
        if (Objects.isNull(inventory.getSupplier())){
            throw new IllegalStateException("Supplier information must be provided.");
        }
    ...
    @Transactional
    public void updateInventory(Integer productId, String name, Float itemPrice, Integer supplierId, String supplierName, String supplierLocation) {
        ...
        if (supplierId % 10 != 0){
            throw new IllegalStateException("Supplier id " + supplierId + " for item #" + productId + " must be a multiple of 10.");
        }
